...
function GetULDDetailsforVCT() {
    ...
    inputxml = '<Root><VCTNo>' + VCTNo + '</VCTNo><CompanyCode>' + CompanyCode + '</CompanyCode><AirportCity>' + AirportCity + '</AirportCity></Root>';
...
function SaveDetails() {
    ...
    if (AcceptanceText == "AWB") {
        ...
        if ((ULD_MODE > 0) && ($('#ddlAcceptance').val() == "1")) {
            var inputULD = '<ROOT><ULDData ULDSeqNo="0" ULDType="' + $('#txtULDType').val().toUpperCase() + '" ULDNumber="' + $('#txtULDNumber').val() + '" ULDOwner="' + $('#txtOwner').val().toUpperCase() + '" Loc="' + strLocationCode + '"/></ROOT>';
        }
        else {
            var inputULD = '<ROOT><ULDData ULDSeqNo="' + $('#ddlULDNo').find('option:selected').val() + '" Loc="' + strLocationCode + '" ULDCondCode="' + ULDCondCode + '"/></ROOT>';
        }
        //var inputULD = '<ROOT><ULDData ULDSeqNo="' + $('#ddlULDNo').find('option:selected').val() + '" Loc="' + strLocationCode + '"/></ROOT>';
...
function HHTGetDriverDetails() {


    var inputxml = '<Root><DriverType>D</DriverType><AirportCity>' + AirportCity + '</AirportCity></Root>';
    ...
    if (errmsg == "" && connectionStatus == "online") {
        $.ajax({
            ...
            url: GHAImportFlightserviceURL + "HHTGetDriverDetails",
            data: JSON.stringify({
                'InputXML': inputxml,
            }),
            contentType: "application/json; charset=utf-8",
            ...
            success: function (Result) {
                ...
                if (str != null && str != "" && str != "<NewDataSet />") {
                    ...
                    $(xmlDoc).find('Table').each(function (index) {
                        DriverDet = $(this).find('DriverDet').text();
                        license_plate_no = $(this).find('license_plate_no ').text();

                        var newOption = $('<option></option>');
                        newOption.val(license_plate_no).text(DriverDet);
                        newOption.appendTo('#ddlDriverList');
                    });
...
function GetAutoULDAWBDetailsByDriverDetails(license_plate_no) {
    ...
    var VCTNo = $('#txtVCTNo').val();
    //inputxml = '<Root><FlightSeqNo>' + flightSeqNo + '</FlightSeqNo><UlDSeqNo></UlDSeqNo><AirportCity>' + AirportCity + '</AirportCity></Root>';
    inputxml = '<Root><VCTNo>' + license_plate_no + '</VCTNo><CompanyCode>' + CompanyCode + '</CompanyCode><AirportCity>' + AirportCity + '</AirportCity></Root>';
